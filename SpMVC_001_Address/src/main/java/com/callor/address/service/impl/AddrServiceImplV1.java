...
public class AddrServiceImplV1 implements AddrService {
	 ...
	 * 객체를 선언하고, 클래스의 생성자를 통해서 객체를 초기화하는
	 * java의 전통적인 객체생성 코드와 대비되는 Spring Framework Project 의 독특한 기법
	 * 
	 * 객체를 선언만 하고, 의존성 주입 절차를 수행한면 Spring Container 에 의해 이미 생성되어
	 * 준비된 객체를 주입해주는 기능
	 * 
	 * 의존성 주입하는 방법
	 * 1. 필드 주입
	 ...
	 * 이 방법이 메모리 관리 측면에서 문제가 있다는 보고가 있으면서 사용이 점차 줄어 들고 있다
	 * 이 방법은 메모리 누수를 줄이기 위하여 객체를 private 으로 선언할 것을 권장하는 방법이다
	 * 객체를 private 으로 선언하면 상속(extends)을 했을때 다시 객체를 선언해야 하는 문제가 발생한다
	 * 
	 * 참고 
	 * 메모리 누수 정도 : public > protected > private
	 */
	@Autowired
	...
	/*
	 * 생성자를 통해서 주입될 객체는 final로 선언을 한다
	 * 이 방식의 특징은 한번 주입이 되면, 코드가 작동되는 중에 또 다시 주입을 할 수 없다.
	 * 객체 보호가 완전하다.
	 * 객체의 순환참조가 없다.
	 * 이 방식은 메모리 누수가 없고, 객체가 보호되며, 순환참조에 의한 코드의 exception 을 막는다.
	 * @Autowired를 붙이지 않아도 된다
	 * 
	 * 이 방식을 사용하려면 반드시 생성자 method 가 있어야 한다
	 * 코드가 다소 번잡해 질 수 있다
	 * 실수로 생성자 method를 만들지 않았을 경우 NullPointer exception 발생
	 */
	 ...
	 * method 가 public으로 완전 개방된 상태이기 때문에
	 */
