...
public class HomeController {
	 ...
	 * 2개 이상의 Component 들이 같은 Service interface 를 상속받아 생성된 경우
	 * 이 Component 를 주입받아 사용하는 곳에서는 어떤 Component 를 주입받아 사용해야 하는지 알수가 없다
	 * 
	 *  이땐느 @Component ("이름")에 이름을 부여하고
	 *  사용할 곳에서 주입받을떄 @Qualifier()를 통햇 명시적으로 Component를 주입받을지 지정해 주어야 한다
	 */
...
//		List<BBsDto> bbsList = bbsDao.selectAll();
	 ...
	 * bbsDto 가 null 인 경우
	 * 어딘가에 @ModelAttribute("BBS")라는 이름이 부착된 method를 찾는다
	 * 만약 method 가 발견되면 자동으로 해당 method를 호출하여 데이터를 준비해 준다
	 */
...
//		model.addAttribute("BBS", bbsDto);
	 ...
	 * Client(Browser)에서 b_username 이라는 변수에 값이 담겨서 전달되어 오면
	 * username 이라는 변수에 값을 저장하여 method 내부로 전달하라
	...
	/*
	 * 매개 변수 설명
	 * BBsDto bbsDto : input tag에 입력한 게시판 관련 text를 받는 곳
	...
	public String insert(BBsDto bbsDto, 
		...
		try {
			...
			// 멀티파일 업로드가 실행되었을때 마네 filesUp() 호출
			// 파일 개수 세는 방법 
			//getFile.getSize() b_image 를 List로 변환하고 개수 세기
