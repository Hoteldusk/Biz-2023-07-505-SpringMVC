document.addEventListener("DOMContentLoaded", () => {
  ...
  const bnameSearch = async (e) => {
    ...
    // ?변수=값으로 보내는 데이터 : queryString 방식의 요청
    ...
    // result 배열(List) 전체를 반복 순회하면서
    // 각각의 요소를 book에 담아 내부의 함수로 전달하라
    result.forEach((book) => {
      // console.table(book);

      // div tag를 새로 생성하기
      const book_item = document.createElement("div");
      // 새로 생성된 div의 innerHTML 속성을 통하여 문자열 보이기
  ...
  키보드 관련 event
  keydown : 키가 눌리는 순간
        Enter, Esc 등 특수키값을 Capture 할 수 있다
  keypress : keydown 거의 유사
  keyup : 키보드에서 손이 떼어지는 순간
        숫자, 영문자 등 키보드에 표시된 문자를 Capture
  */
