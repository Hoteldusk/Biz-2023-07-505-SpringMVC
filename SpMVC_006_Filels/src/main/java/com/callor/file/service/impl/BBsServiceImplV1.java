...
public class BBsServiceImplV1 implements BBsService {
	 ...
	 * 
	 * @Bean : 객체를 생성하여 컨테이너에 보관해 달라 라는 요청 원래 @Bean의 역할과 달리 TABLE을 생성하기 위한 코드로 변형
	 */
	@Autowired
	...
	/*
	 * 파일포함한 Update 1. 새롭게 업로드된 대표파일과 원래 Table 에 저장된 파일이름 비교 2. 업로드된 파일과 Table 에 저장된
	 * 파일이 서로 다르면 업로드된 파일 삭제 3. 새로운 파일 업로드 4. 업로드된 새로운 파일정보 Table에 저장
	 */
	@Override
